...
sai_status_t sai_create_hostif(_Out_ sai_object_id_t *hif_id,
  ...
  *hif_id = switch_api_hostif_create(device, &hostif);
  status = (*hif_id == SWITCH_API_INVALID_HANDLE) ? SAI_STATUS_FAILURE
                                                  : SAI_STATUS_SUCCESS;
    ...
    switch (attribute->id) {
      ...
      case SAI_HOSTIF_TRAP_ATTR_TRAP_CHANNEL:
        rcode_api_info.channel =
            switch_sai_channel_to_switch_api_channel(attribute->value.u32);
        break;
...
*    [in,out] buffer_size - [in] allocated buffer size. [out] actual packet size
*in bytes
...
bool switch_sai_tx_type_to_switch_api_tx_type(sai_hostif_tx_type_t tx_type) {
  bool bypass = TRUE;
  switch (tx_type) {
